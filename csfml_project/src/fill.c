...
linked_list_t *fill_ducks(game_t *hunter)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        (sfVector2f){-150, rand() % 400}, (sfVector2f){RESIZE_X, RESIZE_Y});
...
void fill_backgrounds(game_t *hunter)
{
    ...
    hunter->backgrounds[BACKGROUND] = create_sprite(texture, (sfIntRect){0, 0, 800, 600},
    (sfVector2f){0, 0}, (sfVector2f){RESIZE_X, RESIZE_Y});
    sfTexture *texture2 = create_texture("./assets/img/foreground.png", hunter, sfTrue);
    hunter->backgrounds[FOREGROUND] = create_sprite(texture2, (sfIntRect){0, 0, 800, 600},
    (sfVector2f){0, 0}, (sfVector2f){RESIZE_X, RESIZE_Y});
    sfTexture *texture3 = create_texture("./assets/img/interface/shots.png", hunter, sfTrue);
    hunter->backgrounds[BACK_LIFE] = create_sprite(texture3, (sfIntRect){0, 0, 26, 17},
    (sfVector2f){72, 517}, (sfVector2f){RESIZE_X, RESIZE_Y});
    hunter->backgrounds[LIFE] = create_sprite(texture3, (sfIntRect){26, 0, 26, 17},
    (sfVector2f){72, 517}, (sfVector2f){RESIZE_X, RESIZE_Y});
}
...
void fill_struct(game_t *hunter)
{
    ...
    sfRenderWindow_setFramerateLimit(hunter->window, FPS);
    hunter->event = (sfEvent){0}; hunter->textures = NULL;
    ...
    sfSprite_setOrigin(hunter->cursor, (sfVector2f){12, 11});
    sfRenderWindow_setMouseCursorVisible(hunter->window, sfFalse);
    hunter->speed = 1; hunter->score_nb = 0; hunter->score = sfText_create();
    ...
    hunter->shoot_sound = sfSound_create(); hunter->hit_sound = sfSound_create();
    ...
    fill_backgrounds(hunter); hunter->ducks = fill_ducks(hunter); hunter->clock = sfClock_create();
    hunter->animation_clock = sfClock_create();
}
